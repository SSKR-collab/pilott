#!/usr/bin/env python3
"""
Generate Distributed RockNet configuration for any UCR dataset.
Produces rocket_config.h and rocket_config.c in distributed_sim/ directory.

Usage:
    python3 generate_distributed_config.py <dataset_name> [num_nodes]

Examples:
    python3 generate_distributed_config.py ECG5000 7
    python3 generate_distributed_config.py FaceAll 7
    python3 generate_distributed_config.py Coffee 7
"""
...
def load_ucr_dataset(name, test=False):
    import os
    data_root = os.environ.get('UCR_DATA_ROOT', str(Path.home() / 'datasets'))
    data = copy.deepcopy(
        pd.read_csv(f"{data_root}/{name}/{name}_{'TRAIN' if not test else 'TEST'}.tsv",
                     sep="\t", header=None))
...
def main(dataset_name, num_nodes=7):
    quantize = True
    np.random.seed(1)

    print(f"Loading {dataset_name} dataset...")
    X_train, y_train = load_ucr_dataset(dataset_name, test=False)
    X_test, y_test = load_ucr_dataset(dataset_name, test=True)
...
/* Number of distributed devices — must match main.c NUM_NODES */
#ifndef NUM_NODES
...
"""
    ...
    c_lines.append('')
    c_lines.append('extern uint16_t TOS_NODE_ID;')
    c_lines.append('')
    c_lines.append('static time_series_type_t const *training_timeseries[NUM_TRAINING_TIMESERIES];')
    c_lines.append('static time_series_type_t const *evaluation_timeseries[NUM_EVALUATION_TIMESERIES];')
    c_lines.append('')
    ...
    c_lines.append('')
    c_lines.append('static float biases[MAX_FEATURES_PER_DEVICE];')
    c_lines.append('')
    ...
    print(f"\nConfiguration generated for {dataset_name} with {num_nodes} nodes.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 generate_distributed_config.py <dataset_name> [num_nodes]")
        print("Example: python3 generate_distributed_config.py FaceAll 7")
        sys.exit(1)

    dataset_name = sys.argv[1]
    num_nodes = int(sys.argv[2]) if len(sys.argv) > 2 else 7
    main(dataset_name, num_nodes)
