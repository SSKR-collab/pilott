/*
 * Dynamic Tree Quantization for QADAM Optimizer
 * Adapted from cp_firmware/app/dynamic_tree_quantization.c
 * Printf debug output removed for simulation
 */
#include <stdint.h>
#include <stdlib.h>
#include <math.h>
...
void init_dynamic_tree_quantization(void)
{
    for (uint16_t q = 0; q < 256; q++) {
        ...
        float mantissa = ((q & ((int)roundf(powf(2, (6 - exponent))) - 1))) /
                         (powf(2, (6 - exponent)) - 1 + 1e-7);
        quantized_values[255 - q] = sign * mantissa * scaling;
    }

    qsort(quantized_values, 256, sizeof(float), compare_float);
    /* Debug printf removed for simulation */
}
